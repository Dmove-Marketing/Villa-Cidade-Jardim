...
(function() {
    function iniciarHeaderScroll() {
        ...
        const hero = document.getElementById('hero-casamentos');

        if (!header || !hero) {
            console.log('Header ou Hero não encontrados.');
            return;
        }

        function verificarScroll() {
            const scrollY = window.scrollY || window.pageYOffset;
            const pontoAtivacao = hero.offsetTop + hero.offsetHeight - 100;

            if (scrollY >= pontoAtivacao) {
