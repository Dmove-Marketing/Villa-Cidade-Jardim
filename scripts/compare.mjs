...
async function captureScreenshot(url, outputPath, context, viewport) {
...
function compareImages(imgAPath, imgBPath, diffOutPath) {
  ...
  // Crop buffers to width/height to avoid length mismatch in pixelmatch
  const dataA = Buffer.alloc(width * height * 4);
  const dataB = Buffer.alloc(width * height * 4);

  for (let y = 0; y < height; y++) {
    imgA.data.copy(dataA, y * width * 4, y * imgA.width * 4, (y * imgA.width + width) * 4);
    imgB.data.copy(dataB, y * width * 4, y * imgB.width * 4, (y * imgB.width + width) * 4);
  }

  const mismatch = pixelmatch(dataA, dataB, diff.data, width, height, {
    threshold,
    includeAA: false,
    diffColor: [220, 38, 38],
    aaColor:   [251, 191, 36],
...
async function main() {
  ...
  for (const vp of VIEWPORTS) {
    ...
    try {
      ...
      await Promise.all([
        captureScreenshot(prodUrl,  prodPath,  context, vp),
        captureScreenshot(localUrl, localPath, context, vp),
      ]);
      ...
      console.error(`    ❌ Falha no viewport ${vp.name}:`, err.message || err);
