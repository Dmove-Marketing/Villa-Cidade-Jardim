...
function discoverPages() {
  ...
  // Filtra a Home por padrão caso queira ignorar, mas permite rodar caso deseje
...
async function main() {
  ...
  for (const page of pages) {
    ...
    try {
      // Executa o Lighthouse em modo Headless
      const cmd = `npx -y lighthouse "${url}" --chrome-flags="--headless --no-sandbox" --output=json --output-path="${reportPath}"`;
      execSync(cmd, { stdio: 'inherit' });
      auditSuccess = true;
    } catch (err) {
      // Captura o término devido ao bloqueio temporário de pastas no Windows (EPERM)
    ...
    // Delay de 2 segundos entre execuções para liberação de portas pelo Windows
    await sleep(2000);
  }

  // Finaliza o servidor local se foi este script que o iniciou
  if (serverProcess) {
    console.log(`\n🛑 Encerrando o servidor local em segundo plano...`);
    // No Windows, mata a árvore de processos do cmd/npm/astro
